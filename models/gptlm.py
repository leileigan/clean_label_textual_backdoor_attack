...
import numpy as np
class GPT2LM:
    def __init__(self, pre_model_path:str, use_tf=False, device=None, little=False):
        """
        ...
        import transformers
        self.use_tf = use_tf
        self.tokenizer = transformers.GPT2TokenizerFast.from_pretrained(pre_model_path)

        if use_tf:
            self.lm = transformers.TFGPT2LMHeadModel.from_pretrained(pre_model_path)
        else:
            self.lm = transformers.GPT2LMHeadModel.from_pretrained(pre_model_path, from_tf=False)
            self.lm.to(device)
